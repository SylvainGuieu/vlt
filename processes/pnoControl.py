...
""", bufferReader=buffreader.getreader("ONLINE")),
    "status"	:vlt.Command("STATUS",{"function":vlt.Param("-function", vlt.dtypeFunctionListMsg, "%s"),"expoId":vlt.Param("-expoId", int, "%d")},helpText="""Get the status of the functions in the list of arguments (default: get the
...
class pnoControl(vlt.Process):
    """

This is a pnoControl class vlt.Process automaticaly generated from file
     /Users/guieu/python/vlt/CDT/pnoControl.cdt
...
proc.help() will return help for every commands
    ...
    """
    commands = pnoControl_commands
    for c in pnoControl_commands: exec("%s = msgdef('%s',pnoControl_commands)"%(c,c))

proc = pnoControl("pnoControl")
